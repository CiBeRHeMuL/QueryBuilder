...
// region MODULE_CONTRACT [DOMAIN(8): Merge; CONCEPT(8): WhenNotMatchedBySourceClause; TECH(7): Clause]
/**
 * @moduleContract
 * @purpose Wrap a WHEN NOT MATCHED BY SOURCE action with optional AND conditions. Provides static factories ::update() and ::delete().
 *         Accepts MergeWhenMatchedActionInterface (same allowed actions as WHEN MATCHED: UPDATE, DELETE, DO NOTHING).
 *         Renders itself via getSql().
 * @scope PgSQL-specific MERGE clause value object.
 * @input MergeWhenMatchedActionInterface $action, array $and
 * @output Immutable clause DTO for PgSQL MERGE query.
 * @invariants
 * - $action implements MergeWhenMatchedActionInterface, NOT MergeWhenNotMatchedActionInterface.
 * @modulemap
 * MergeWhenNotMatchedBySourceClause => WHEN NOT MATCHED BY SOURCE clause wrapper
 */
// endregion MODULE_CONTRACT
// GREP_SUMMARY: MergeWhenNotMatchedBySourceClause, WHEN NOT MATCHED BY SOURCE, MERGE, PgSQL, clause, static factory
// STRUCTURE: ▶ ┌$action, $and┐ + ::update(set, and) + ::delete(and) + getSql → ∑ [MergeWhenNotMatchedBySourceClause]

// region CLASS_MergeWhenNotMatchedBySourceClause [DOMAIN(8): Merge; CONCEPT(8): WhenNotMatchedBySourceClause; TECH(7): Clause]
/**
 ...
 *
 * @purpose Final readonly DTO for WHEN NOT MATCHED BY SOURCE (PostgreSQL extension) clause.
 *         Uses MergeWhenMatchedActionInterface — same action set as WHEN MATCHED.
 *         Renders as "WHEN NOT MATCHED BY SOURCE [AND cond] THEN action" via getSql().
 */
readonly class MergeWhenNotMatchedBySourceClause
{
    /**
    ...
    /**
     * @purpose Render this clause as "WHEN NOT MATCHED BY SOURCE [AND cond] THEN action".
     * @complexity 4
     * STRUCTURE: ┌'WHEN NOT MATCHED BY SOURCE', ┌AND cond?┐, 'THEN', action.getSql┐ → ● merge → ∑ Expr
     *
    ...
    {
        $parts = ['WHEN NOT MATCHED BY SOURCE'];
        $params = [];
    ...
    /**
     * @purpose Create a WHEN NOT MATCHED BY SOURCE THEN UPDATE SET clause.
     * @complexity 2
     *
    ...
    /**
     * @purpose Create a WHEN NOT MATCHED BY SOURCE THEN DELETE clause.
     * @complexity 1
     *
     * @param TConditions $and optional AND conditions
     *
...
// endregion CLASS_MergeWhenNotMatchedBySourceClause
